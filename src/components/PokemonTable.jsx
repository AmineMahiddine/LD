...
function PokemonTable() {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - filteredPokemon.length) : 0;
  ...
  return (
    <div>
        ...
        {/* <SearchIcon/> */}
      <TextField id="outlined-basic" label="Search..." variant="outlined" value={searchName}
      ...
      <TextField
        type="number"
        id="outlined-basic" label="Power threshold" variant="outlined" value={minPower}
        onChange={(e) => setMinPower(parseInt(e.target.value))}/>
        
      <div>
        <p>Min Power: {currentMinPower}</p>
        <p>Max Power: {currentMaxPower}</p>
      </div>
